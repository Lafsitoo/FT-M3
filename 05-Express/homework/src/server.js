...
server.delete("/posts", (req, res) => {
    const { id } = req.body;
    if(!id){
        return res.status(STATUS_USER_ERROR).json({error: "Te olvidaste el id crack"})
    }
    const post = posts.find(p => p.id === id)
    if(!post){
        return res.status(STATUS_USER_ERROR).json({error: "No mi ciela"})
    }
...
server.delete("/author", (req, res) => {
    ...
    if(!author){
        return res.status(STATUS_USER_ERROR).json({error: "Sin author no sigo"})
    }
